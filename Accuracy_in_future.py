...
cf_preds_df = cf_preds_df.drop(columns=['Стать_клієнта'])
#print(cf_preds_df)
...
def for_future(predictions, true_values, k=10):
    #Якщо видалити Стать клієнта із дослідження точності то точність падає не сильно на 2%+-
    #predictions=predictions.drop(columns="Стать_клієнта")
    #true_values=true_values.drop(columns="Стать_клієнта")
    """
...
future = for_future(cf_preds_df,test_,k=10)
print("Future (Prediction for splited df) - ",future)
